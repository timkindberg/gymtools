...
- **Today** — shows the day's workout (Mon = Day A, Wed = Day B, Fri = Day C),
  a last-session recap, and quick stats (weekly consistency, streak).
- **Guided sessions** — a quick 5-slider symptom check-in first (knee, shoulder,
  neck/headache, energy, sleep). If a joint is flaring, the app warns you and
  points to the safer swaps. Then you go exercise-by-exercise: coach's notes,
  cues, injury flags, alternatives, your **last performance**, an
  **auto-progression suggestion**, a set grid to log weight × reps, and a
  built-in **rest timer** (with sound + vibration).
...
- **Progress** — strength charts (estimated 1-rep-max & volume) per exercise,
  **symptom trend lines** over time, and your full session log.
...
Full-body, 3×/week (~60 min), designed around the specific constraints you gave:

| Constraint | How the program responds |
|---|---|
| Leg-length discrepancy | Unilateral lower work, anti-lateral-flexion carries, no ego-loaded axial maxes |
| Crooked / asymmetric posture | Heavy horizontal pulling, scapular control, anti-rotation core |
| Tight hamstrings | Hip-hinge patterning started light, range progressed slowly, mobility work |
| Right shoulder wear | Neutral-grip horizontal pressing, landmine angles, big dose of cuff + face pulls — **no** overhead press, upright rows, or behind-the-neck |
| Right knee torn meniscus | Box-limited squats, quad (VMO) focus, strong posterior chain, no deep loaded flexion or twisting under load |
| Cervicogenic migraines | **No** heavy trap loading; deep-neck-flexor + cervico-scapular work; breathe, never grind |
